...
def extract_voice_camera(text: str) -> str | None:
    categories = ["voice", "camera"]
    prompt = (
        f"Which category from the list {categories} fits the sentence "
        f"'{text}' best? Only reply with the matching category name."
    )
    ...
    # find which keyword appeared
    return next((c for c in categories if c in llm_reply.lower()), None)
...
def extract(input_type, text):
    match input_type:
        ...
        case INPUT_TYPE.VOICE_CAMERA:
            return extract_voice_camera(text)
